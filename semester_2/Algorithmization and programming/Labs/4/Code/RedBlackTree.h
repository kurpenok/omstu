...
public:
  ...
  void
  insert(T key); // Вставляем узел, ключ это значение ключа, внешний интерфейс
...
private:
  ...
  void insert(RBTNode<T> *&root,
              RBTNode<T> *node); // Вставляем узел, внутренний интерфейс
...
template <class T> void RBTree<T>::insert(RBTNode<T> *&root, RBTNode<T> *node) {
  RBTNode<T> *x = root;
  RBTNode<T> *y = NULL;
  while (x != NULL) {
    y = x;
...
template <class T> void RBTree<T>::remove(RBTNode<T> *&root, RBTNode<T> *node) {
  ...
  if (node->left != NULL && node->right != NULL) {
    ...
    // Найти узел-преемник (самый нижний левый узел правого поддерева текущего
    // узла)
    replace = node->right;
    ...
    // Ситуация с корневым узлом
      ...
      root = replace;
    // child - это правильный узел, который заменяет узел и является узлом,
    // который требует последующей корректировки Поскольку замена является
    // преемником, он не может иметь левого дочернего узла Аналогично, у
    // узла-предшественника не может быть правого дочернего узла
    child = replace->right;
  ...
  // Когда в удаленном узле только левый (правый) узел пуст, найдите дочерний
  // узел удаленного узла
  if (node->left != NULL)
    child = node->left;
  else
    child = node->right;
...
template <class T> RBTNode<T> *RBTree<T>::search(T key) {
...
template <class T> void RBTree<T>::print(RBTNode<T> *node) const {
  if (node == NULL)
    return;
  if (node->parent == NULL)
    cout << node->key << "(" << node->color << ") is root" << endl;
  else if (node->parent->left == node) {
    cout << node->key << "(" << node->color << ") is " << node->parent->key
         << "'s " << "left child" << endl;
  } else {
    cout << node->key << "(" << node->color << ") is " << node->parent->key
         << "'s " << "right child" << endl;
